...
### 实验心得
---
通过本次数据库实验，我掌握了MySQL中数据库和数据表的基本操作方法，包括创建数据库、创建数据表、插入数据以及使用SELECT语句进行数据查询等内容。虽然实际操作中，也遇到了一些小问题，比如在插入“name”数据时没有给字符串加引号，导致出现“Unknown column”的报错。通过查阅资料，我了解到字符串类型插入时必须使用单引号括起来。在进行查询操作时，我也对上课提到的from、where语法结构有了更清晰的认识。