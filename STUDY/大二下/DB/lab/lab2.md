...
你已经完成了数据库创建和表的建立部分，下面我帮你把整个 lab 的剩余部分补充完整，包括索引、视图、数据插入、各种查询、视图操作等。你可以按顺序执行这些命令并截图，最后整理成实验报告。

注意：你建表时有一个**顺序问题** —— `depositor` 引用了还没创建的 `customer` 和 `account`，`borrow` 引用了还没创建的 `loan`。MySQL 的 InnoDB 在创建时就会检查外键，所以这些 `CREATE TABLE` 实际上应该是失败的。建议先 `DROP` 掉重建，正确顺序是：`branch → customer → account → loan → depositor → borrow`。
...
```sql
-- 添加列（你已经做过 manager，再演示一下修改和删除）
ALTER TABLE branch ADD manager VARCHAR(50);
