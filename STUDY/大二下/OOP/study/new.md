...
p是多态变量，要构造的是Derived类，但是Derived是Base的子类，然后因为Base没有声明构造函数，所以new的时候先调用Base的默认构造函数然后调用Derived自己的构造函数，析构的时候因为Base的析构函数是虚函数，所以会直接调用Derived自己的析构函数（但是derived类内没有声明析构函数，程序就会调用自动默认析构函数）
```
