# Instruction-Level Parallelism (ILP)[¶](https://note.hobbitqia.cc/CA/CA4/#instruction-level-parallelism-ilp "Permanent link")

 约 3250 个字  19 行代码  29 张图片  预计阅读时间 11 分钟

## Dynamic Scheduling[¶](https://note.hobbitqia.cc/CA/CA4/#dynamic-scheduling "Permanent link")

Example

前两条指令存在数据依赖，DIV 语句的执行时间一般较长。那么前两条指令就会等待除法完成，顺序执行里面后面的指令也会跟着等（但其实和除法没有关系），这样就造成了浪费。

![](https://cdn.hobbitqia.cc/20231110145255.png)

- Idea: Dynamic Scheduling
- Method: out-of-order execution

![](https://cdn.hobbitqia.cc/20231110145544.png)
...
- **Issue(IS)**: Decode instructions, check for _structural hazards_. (in-order issue)
- **Read Operands(RO)**: Wait until no _data hazards_, then read operands. (out of order execution)

![](https://cdn.hobbitqia.cc/20231110150243.png)
...
### Scoreboard algorithm[¶](https://note.hobbitqia.cc/CA/CA4/#scoreboard-algorithm "Permanent link")

The basic structure of a processor with scoreboard:

![](https://cdn.hobbitqia.cc/20231110155959.png)
...
`[](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-1)FLD F6, 34(R2) [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-2)FLD F2, 45(R3) [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-3)FMUL.D F0, F2, F4 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-4)FSUB.D F8, F2, F6 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-5)FDIV.D F10, F0, F6 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-0-6)FADD.D F6, F8, F2` 

- Instrucion Status:
    
    ![](https://cdn.hobbitqia.cc/20231110161000.png)
...
- Function Component Status:
    ...
    ![](https://cdn.hobbitqia.cc/20231110161420.png)
...
- Register Status
    ...
    ![](https://cdn.hobbitqia.cc/20231110162034.png)
...
- It tracks when operands for instructions are available to minimize _RAW_ hazards;
- It introduces _register renaming_ in hardware to minimize _WAW_ and _WAR_ hazards.
...
- **Issue**: Get the next instruction from the head of the instruction queue (FIFO)
    ...
    - If there is a matching _reservation station_ that is empty, issue the instruction to the station with the operand values, if they are currently in the registers.
...
`[](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-1)FLD F6, 34（R2） [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-2)FLD F2, 45（R3） [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-3)FMUL.D F0, F2, F4 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-4)FSUB.D F8, F2, F6 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-5)FDIV.D F10, F0, F6 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-1-6)FADD.D F6, F8, F2` 

![](https://cdn.hobbitqia.cc/20231110171456.png)

最左侧为保留站名，Qj、Qk 表示数据依赖于哪一条指令。

![](https://cdn.hobbitqia.cc/20231110172122.png)

过了一段时间，下一个状态：

![](https://cdn.hobbitqia.cc/20231110172334.png)

![](https://cdn.hobbitqia.cc/20231110172518.png)
...
## Hardware-Based Speculation[¶](https://note.hobbitqia.cc/CA/CA4/#hardware-based-speculation "Permanent link")
...
![](https://cdn.hobbitqia.cc/20231117201549.png)

ROB 需要知道我们是从哪条指令来的，标 busy 的是当前正在执行的指令（即还没有 commit）

![](https://cdn.hobbitqia.cc/20231117201602.png)

Practice in Class

![](https://cdn.hobbitqia.cc/20231117201850.png)

实际上加上 ROB 之后，前面的执行和 Tomasulo’s Approach 是一样的，只是多了一个 commit 状态，在后面加一列，需要按照流出顺序提交即可。

![](https://cdn.hobbitqia.cc/20231117204124.png)
...
## Exploiting ILP Using Multiple Issue and Static Scheduling[¶](https://note.hobbitqia.cc/CA/CA4/#exploiting-ilp-using-multiple-issue-and-static-scheduling "Permanent link")
...
![](https://cdn.hobbitqia.cc/20231215172006.png)

### based on static scheduling[¶](https://note.hobbitqia.cc/CA/CA4/#based-on-static-scheduling "Permanent link")
...
- Assumption: Two instructions flow out every clock cycle:
    - _1 integer instruction + 1 floating-point operation instruction_
- Among them, _load_, _store_ and _branch_ instructions are classified as _integer instructions_.

![](https://cdn.hobbitqia.cc/20231215173139.png)

### based on dynamic scheduling[¶](https://note.hobbitqia.cc/CA/CA4/#based-on-dynamic-scheduling "Permanent link")
...
`[](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-1)Loop:  [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-2)FLD    F0, 0 (R1)   // Take an array element and put it into F0 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-3)FADD.D F4, F0, F2   // add the scalar in F2 [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-4)FSD    F4, 0 (R1)   // storeresult [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-5)ADDI   R1, R1, 8    // increment pointer by 8 //(each data occupies 8 bytes) [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-6)BNE    R1, R2, Loop // If R1 is not equal to R2, it means it is  [](https://note.hobbitqia.cc/CA/CA4/#__codelineno-2-7)                    //not over yet, move to Loop to continue`

对于没有分支预测的情况：可以看到分支后面的一条指令，在周期 7 才能执行（即周期 6 分支指令执行结束）

![](https://cdn.hobbitqia.cc/20231215173427.png)
...
### Very long instruction word technology(VLIW)[¶](https://note.hobbitqia.cc/CA/CA4/#very-long-instruction-word-technologyvliw "Permanent link")
...
### Superpipelining processor[¶](https://note.hobbitqia.cc/CA/CA4/#superpipelining-processor "Permanent link")

A pipeline processor with 8 or more instruction pipeline stages is called a **superpipelining processor**.

在一个很小的  时间（小于一个阶段的用时）后就发射下一条指令。

![](https://cdn.hobbitqia.cc/20231215193400.png)

本质是流水线的细分。

![](https://cdn.hobbitqia.cc/20231215193536.png)

2024年1月20日 14:12:23 2023年11月10日 17:39:14
