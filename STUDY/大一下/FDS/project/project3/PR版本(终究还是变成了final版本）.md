...
```c
    ...
    min=MAXWEIGH,adr=-1
	for i:=0 to H->size-1
		if H->T[i]->weight<min&&the vertex isn't be used then
			min=H->T[i]->weight;
            adr=i;
     ...
     i=adr
     while 2*i+1<H->size 
		select the min child vertex:smallest
...
```c
    ...
    for i from 1 to nv:
        ...
        if i==1:
            list[s[i]][0]=0
        else if s[i]->weight is not the smallest:
            free list
            return 0
        for j from 1 to nv:
