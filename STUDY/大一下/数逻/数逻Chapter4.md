# Chap 4 Sequential Circuits[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#chap-4-sequential-circuits "Permanent link")

 约 6345 个字  预计阅读时间 32 分钟
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/79.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/79.png)

> 其中，从给定输入到更新输出有一个 tGtG​ 的延时。
...
## 锁存器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E9%94%81%E5%AD%98%E5%99%A8 "Permanent link")

### SR & S'R' 锁存器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#sr--sr-%E9%94%81%E5%AD%98%E5%99%A8 "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/80.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/80.png)
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/82.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/82.png)

> 注意！SR 锁存器通过 或非门 实现，但带输入控制的 SR 锁存器则通过 **与非门** 实现！

---

### D 锁存器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#d-%E9%94%81%E5%AD%98%E5%99%A8 "Permanent link")

D 锁存器实际上是带控制输入的 SR 锁存器的改进。其中我们关注到，虽然 S 和 R 起作用的有三种状态

我们关注到，即使是带控制输入的 SR 锁存器，也存在一个「undefined」状态，而这是我们不希望出现的，于是我们想，能不能避免这种情况的发生。这时候我们观察到，由于可以通过 C 来控制是否保持，所以可以直接强制要求 S=R‾S=R，于是就避免了 undefined 情况的出现，这就是 D 锁存器(D Latch)。

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/83.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/83.png)
...
当然，D 锁存器也存在问题。如果我门将 Q‾Q​ 接到 DD 上，就会发现 C 置 `1` 时 D 锁存器将不停的变化其中的数据，出现 **空翻**，导致我们无法确定 C 置 `0` 时 Q 的输出究竟是什么，而这是非常危险的。
...
## 触发器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E8%A7%A6%E5%8F%91%E5%99%A8 "Permanent link")
...
### SR 主从触发器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#sr-%E4%B8%BB%E4%BB%8E%E8%A7%A6%E5%8F%91%E5%99%A8 "Permanent link")

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/84.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/84.png)
...
### 边沿触发式触发器[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E8%BE%B9%E6%B2%BF%E8%A7%A6%E5%8F%91%E5%BC%8F%E8%A7%A6%E5%8F%91%E5%99%A8 "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/85.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/85.png)

> 关注上升沿前后，上升沿前一刻，主锁存器可写，从锁存器只读；上升沿后一刻，主锁存器只读，从锁存器只写，且写入的是主锁存器存储的值——换句话来说，存的是上升沿前一刻写入主锁存器的内容。

需要注意的一点是，D 触发器没有「保持」态。如果要使 D 触发器保持，可以使时钟脉冲失效或通过 `MUX` 将输出接到输入实现。其中前者不太常用，因为受门控的时钟脉冲进入触发器时有延迟，即 **时钟偏移(clock skew)**。关于时钟偏移的更详细介绍可以移步 **[第六章#保持](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap06/#%E4%BF%9D%E6%8C%81)**。

---

### 标准图形符号[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E6%A0%87%E5%87%86%E5%9B%BE%E5%BD%A2%E7%AC%A6%E5%8F%B7 "Permanent link")

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/86.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/86.png)

> 带圆圈表示负逻辑。

---

### 直接输入[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%9B%B4%E6%8E%A5%E8%BE%93%E5%85%A5 "Permanent link")
...
### 触发器的基本描述方法[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E8%A7%A6%E5%8F%91%E5%99%A8%E7%9A%84%E5%9F%BA%E6%9C%AC%E6%8F%8F%E8%BF%B0%E6%96%B9%E6%B3%95 "Permanent link")
...
## 时序电路分析[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E6%97%B6%E5%BA%8F%E7%94%B5%E8%B7%AF%E5%88%86%E6%9E%90 "Permanent link")
...
### 触发器的输入方程[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E8%A7%A6%E5%8F%91%E5%99%A8%E7%9A%84%E8%BE%93%E5%85%A5%E6%96%B9%E7%A8%8B "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/87.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/87.png)

可以发现，其中包含两个触发器 DADA​ 和 DBDB​，以及一个组合逻辑输出 YY，因而可以根据电路的特征，得到下面这几个式子：

DA=AX+BXDB=A‾XY=(A+B)X‾DA​DB​Y​=AX+BX=AX=(A+B)X​

---

### 状态表[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E8%A1%A8 "Permanent link")

当然，时序电路的逻辑有时候也可以通过状态表来描述，只不过与组合电路的 真值表 不同，**状态表(state table)** 有四栏：当前状态(present state)、输入(input)、下一状态(next state)、输出(output)。其含义是比较显然的，只不过需要通过电路图得到状态表，需要首先得到「当前状态」向「下一状态」转移的方程，即对于某个时刻 tt 的触发器 AA 的输出 A(t)A(t)，需要得到它下一刻的状态 A(t+1)=f(A(t),...)A(t+1)=f(A(t),...)。

同样以这个电路为 🌰：

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/87.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/87.png)

首先我们对 DADA​ 得到转移方程：A(t+1)=DA=A(t)X+B(t)XA(t+1)=DA​=A(t)X+B(t)X，可以简写为 A(t+1)=DA=AX+BXA(t+1)=DA​=AX+BX。类似地也能得到 BB 的转移方程。
...
在 **[状态图](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E5%9B%BE)** 中，我们还将再一次遇到这两个人名。

---

### 状态图[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E5%9B%BE "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/89.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/89.png)
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/88.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/88.png)
...
状态图还有进阶形态，之后会提及，就是 **[状态机](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E6%9C%BA)**。

---

### 等价状态[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%AD%89%E4%BB%B7%E7%8A%B6%E6%80%81 "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/120.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/120.png)
...
请参考 **[延时分析](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E5%BB%B6%E6%97%B6%E5%88%86%E6%9E%90)**。

---

## 时序电路的设计[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E6%97%B6%E5%BA%8F%E7%94%B5%E8%B7%AF%E7%9A%84%E8%AE%BE%E8%AE%A1 "Permanent link")

类比组合逻辑电路通过真值表设计电路，（同步）时序电路主要依据状态表（或等价表达，如状态图）来设计电路。实际设计中，我们需要考虑触发器以及其它组合电路的选择与设计：

- 触发器的数量基本取决于整个需求中 状态 的数量，nn 个触发器能够表达 2n2n 个二进制状态；
...
3. 给设计好的状态编码赋值，得到具体的状态表；
    ...
    2. 其中还有一种情况是所需要的状态不能完整填充 2n2n 种情况，需要设计无效状态，这种时候可以不考虑它们；
...
## 状态机[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E6%9C%BA "Permanent link")

尽管在小规模的设计中，状态图表现良好，但是随着设计的系统规模变大，我们需要一个更加泛用的方式来表达类似的时序逻辑。

于是，我们在状态图的基础上进行修改，设计了 **状态机(State-Machine Diagram)** 来做这件事。

状态机混用了米勒型和摩尔型的输出格式，优劣在 **[状态图](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%8A%B6%E6%80%81%E5%9B%BE)** 中有所提及。并且，有限图将状态转移方程也写入，总体上对电路设计更友好。下图是状态机的一个“范式”。

Generic State Diagram Template

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/90.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/90.png)
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/92.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/92.png)

---

当然，非法状态机并不只有这一个判据。不过核心思想就是 **不能有歧义或冲突且需要充分**，更数学的表达是，必须满足如下两个条件：

1.非交：对于每一个状态 SiSi​，它的任意两个出度 (Tij,Tjk)(Tij​,Tjk​) 都不能同时成立，即应有：

∀(Tij,Tik),Tij⋅Tik=0∀(Tij​,Tik​),Tij​⋅Tik​=0

2.充分：对于每一个状态 SiSi​，它的所有出度条件应该涵盖所有可能，即应有：
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/91.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/91.png)

---

## 延时分析[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E5%BB%B6%E6%97%B6%E5%88%86%E6%9E%90 "Permanent link")
...
### 组合电路延时[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E7%BB%84%E5%90%88%E7%94%B5%E8%B7%AF%E5%BB%B6%E6%97%B6 "Permanent link")

这一部分的详细内容已经在 **[上一章](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap03/#%E4%BC%A0%E6%92%AD%E5%BB%B6%E8%BF%9F)** 介绍过了，在这里，为了简化计算过程，我们全部采用 tpdtpd​。

---

### 触发器延时[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E8%A7%A6%E5%8F%91%E5%99%A8%E5%BB%B6%E6%97%B6 "Permanent link")
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/93.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/93.png)
...
### 松弛时间[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E6%9D%BE%E5%BC%9B%E6%97%B6%E9%97%B4 "Permanent link")

实际上这就是给整个电路的一个“容差”时间，给定一个误差，但是由于通过前两者计算出来的是最短时间，所以松弛时间必定非负。（不能倒扣！）

---

### 时序电路延时[¶](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/Chap04/#%E6%97%B6%E5%BA%8F%E7%94%B5%E8%B7%AF%E5%BB%B6%E6%97%B6 "Permanent link")

时序电路的延时计算实际上是为了计算时序电路运作的最大频率以设计时钟频率。所以我们需要算出电路能够稳定工作的最小时钟期。

其计算遵循一定方法：

[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/94.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/94.png)

> 图中 tpd,FFtpd,FF​ 指触发器的 propagation time，tCOMBtCOMB​ 指组合电路的总传播时间（一般情况下 tCOMB=max⁡∑itpd,gateitCOMB​=max∑i​tpd,gatei​​），tsts​ 指触发器的 setup time，tslacktslack​ 表示电路的松弛时间。
...
[![](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/122.png)](https://note.isshikih.top/cour_note/D2QD_DigitalDesign/img/122.png)
